...
public:
    string minWindow(string s, string t) {
        ...
        for(int high = 0; high<s.length(); high++) {
            if(f[s[high]] > 0) count++; //means that this letter is in t   
            f[s[high]]--; //reduce the count for the letter on which we are currently 
            if(count == t.length()) { //if current windows contains all of the letters in t
                while(low < high && f[s[low]] < 0) f[s[low]]++, low++; //move low ahead if its not of any significance
                if(min_length > high-low) min_length = high-(min_start=low)+1; //update the min length
                f[s[low++]]++; //move low ahead and also increment the value
